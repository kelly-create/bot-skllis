    ...
    try:
        response = requests.post(
            ...
            json={
                "model": AI_MODEL,
                "messages": [
                    {"role": "system", "content": """你是一位资深的国际新闻编辑和分析师。
你的任务是将英文新闻翻译成流畅的中文，并进行专业的分析提炼。
要求：
1. 翻译要准确、地道，不能只写"人工智能"这种泛泛的词
2. 每条新闻要有完整的中文标题和简要说明
3. 分析要有深度，指出关键趋势和影响
4. 如果信息不足，要如实说明"""},
...
"""
    ...
    prompt = f"""请对以下关于「{topic_name}」的{len(news_list)}条英文新闻进行专业处理：
...
## 任务一：新闻翻译
将每条新闻翻译成中文，格式如下：
- 每条新闻包含：中文标题 + 一句话概要（20-40字）
- 如果原文信息不足，根据标题和来源推断内容
- 不要只写"人工智能"这种泛泛的词，要翻译具体内容

## 任务二：今日综述（300-400字）
1. 今日{topic_name}领域发生了什么重要事件？
2. 有哪些值得关注的趋势或变化？
3. 对普通人有什么影响或启示？
4. 小鸡的简短点评（一两句话）
