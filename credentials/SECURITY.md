...
# 在 ignoreip 行末添加新IP，用空格分隔

# 方法2：使用sed命令
sed -i 's/ignoreip = \(.*\)/ignoreip = \1 NEW_IP/' /etc/fail2ban/jail.local
...
### ⚠️ 重要提醒
- **小鸡自己的IP已加入白名单**，避免自己被锁在外面
- **主人的5个常用IP已加入白名单**，确保随时可以登录
- 白名单修改后必须重载 `systemctl reload fail2ban`
- 定期检查白名单是否包含所有可信IP

---

*白名单配置更新于 2026-02-09 01:42 UTC*
