...
## Gmail 邮箱配置
- **邮箱账号**: zbobo9001@gmail.com ✅ 已集成
- **认证方式**: 应用专用密码（IMAP/SMTP）
- **功能**: 收件箱读取、邮件发送、搜索、标记已读
- **脚本**: `scripts/gmail_client.py`
- **说明**: 使用独立脚本，不修改 Gateway 配置，避免重启问题
...
- 和风天气 API 已对接并测试通过
- Gmail 邮箱已集成并测试通过（发送+接收）

## 教训记录
- **2026-02-07 重启事故**: 配置 hooks 时添加了无效字段导致 Gateway 无限重启
- **解决方案**: 使用 `openclaw doctor --fix` 修复配置
- **预防措施**: 
  1. 修改配置前检查 schema
  2. 优先使用独立脚本而非 Gateway 配置
  3. 出现问题立即汇报，不静默重试
