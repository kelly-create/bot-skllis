...
- ✅ 多Agent已支持“复核FAIL自动打回重跑”闭环，并带最大返工轮次限制（默认2）防止无限循环。
...
- ✅ Agent Team 已收敛为三角色架构：`Lead Agent(gpt-5.3-codex)` + `@frontend(MiniMax-M2.5)` + `@backend(gpt-5.3-codex)`；默认工作流为 `intelligent_dual`（Lead 评估与调度，前后端执行）。
